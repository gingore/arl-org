...
$sql .= " WHERE r.subcategoryID IN (SELECT subcategoryID FROM ResourcesSubCatg WHERE CID = (SELECT CID FROM ResourcesCatg WHERE CatgName = 'Basic Needs'))";

if (!empty($filter)) {
    $subcategoryID = getSubcategoryID($filter, $conn);
    if ($subcategoryID !== null) {
        $sql .= " AND r.subcategoryID = '$subcategoryID'";
    }
}
...
<head>
    ...
    <title>ArlOrg | Basic Needs</title>
...
<body>
    ...
    <div class="container">
        <div class="hero" style="display: flex; justify-content: center; align-items: center; min-height: 10vh;">
            <div class="hero-text">
                <h1 class="title" style="margin: 0;">Basic Needs</h1>
            </div>
        </div>
        ...
        <div class="resource-container">
            <div class="resource-card-container">
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    echo "<p>No basic needs resources found.</p>";
                }
                ?>
            </div>
        </div>
